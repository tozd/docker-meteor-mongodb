...
# File is used by both Meteor app and tozd/meteor-mongodb images. The latter automatically creates the
...
if [ ! -s "$SETTINGS" ]; then
  echo "Set MONGODB_CREATE_PWD, MONGODB_ADMIN_PWD, MONGODB_OPLOGGER_PWD environment variables in '$SETTINGS'."
  exit 1
fi
...
docker rm "${NAME}" || true
sleep 1
docker run --detach=true --restart=always --name "${NAME}" --volume "${SETTINGS}:/etc/service/mongod/run.settings" --volume "${MONGODB_LOG}:/var/log/mongodb" --volume "${MONGODB_DATA}:/var/lib/mongodb" tozd/meteor-mongodb
